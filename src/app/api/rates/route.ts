...
import { getChangeNowRate, getChangeNowRateFix } from '@/lib/rate-adapters/change-now';
import { getChangellyRate, getChangellyRateFix, getChangellyRateRevert } from '@/lib/rate-adapters/changelly';
import { getEasyBitRate } from '@/lib/rate-adapters/easy-bit';
import { getLetsExchangeRate, getLetsExchangeRateFix, getLetsExchangeRateRevert } from '@/lib/rate-adapters/lets-exchange';
...
export async function GET(request: NextRequest) {
    try {
        ...
        const options = Object.fromEntries(searchParams.entries()) as unknown as Options;
        
        const rate = await getRate(options);
        
        return NextResponse.json(rate);
    } catch (error) {
        console.error("Error from getRate route:", error);
